...
class Packet(object):
    ...
    def __init__(self, data_count: int, checksum: str):
        ...
        logger.debug("Initialized packet: {packet}", packet=self)
    ...
    def as_string(self) -> str:
        ...
        """
        logger.debug("Returning bytes {_bytes} as ascii string", _bytes=self._bytes)
    ...
    def as_bytes(self) -> bytes:
        ...
        """
        logger.debug("Returning ascii string {_ascii} as bytes", _ascii=self._ascii)
        if self._ascii is not None:
            _replace_whitespace = self._ascii.replace(" ", "")
            _replace_stx = _replace_whitespace.replace("STX", STX.decode())
            _replace_null = _replace_stx.replace("NULL", NULL.decode())
    ...
    def as_hex(self) -> str:
        ...
        """
        logger.debug("Returning bytes {_bytes} as hex string", _bytes=self._bytes)
        if self._bytes is not None:
...
class ErrorPacket(Packet):
    ...
    def __str__(self) -> str:
        return "{self.__class__}(error_code={self.error_code}, error_description={self.error_description} data_count={self.data_count}, checksum={self.checksum}, _bytes={self._bytes}, _ascii={self._ascii}, _hex={self._hex})".format(
            self=self
        )
...
class ResponsePacket(Packet):
    ...
    def __str__(self) -> str:
        return "{self.__class__}(command={self.command}, acknowledge={self.acknowledge}, data_count={self.data_count}, checksum={self.checksum}, _bytes={self._bytes}, _ascii={self._ascii}, _hex={self._hex})".format(
            self=self
        )
...
class ReplyPacket(Packet):
    ...
    def __init__(self, setting, value, data_count, checksum):
        ...
        try:
            self.value_description = settings_dict[setting]["range"][value]["name"]
        except (KeyError, TypeError):
    ...
    def fromraw(self, raw: bytes):
        ...
        _value = match.group("val").replace(NULL, UNDERSCORE).decode()
        _checksum = match.group("chk").decode()
        return self(_setting, _value, _data_count, _checksum)

    def __str__(self) -> str:
        return "{self.__class__}(setting={self.setting}, setting_description={self.setting_description}, value={self.value}, value_description={self.value_description}, data_count={self.data_count}, checksum={self.checksum}, _bytes={self._bytes}, _ascii={self._ascii}, _hex={self._hex})".format(
            self=self
        )
...
class QueryPacket(Packet):
    ...
    def __init__(self, setting, checksum=None):
        if hex_matcher.match(setting):
            ...
            raise ValueError(
                "setting must consist of two digits ranging from A-F or 0-9"
            )
    ...
    def __str__(self) -> str:
        return "{self.__class__}(setting={self.setting}, setting_description={self.setting_description}, data_count={self.data_count}, checksum={self.checksum}, _bytes={self._bytes}, _ascii={self._ascii}, _hex={self._hex})".format(
            self=self
        )
    ...
    def as_string(self) -> str:
        ...
        Returns:
            str: packet as ASCII string
        """
        return "STX 2 0 {self.__dtc1__} {self.__dtc2__} {self.__id1__} {self.__id2__} NULL {self.__cs1__}{self.__cs2__}ETX".format(
            self=self
        )
...
class CommandPacket(Packet):
    ...
    def as_string(self) -> str:
        ...
        Returns:
            str: packet as ASCII string
        """
        return "STX 3 0 {self.__dtc1__} {self.__dtc2__} {self.__id1__} {self.__id2__} NULL {self.value} NULL {self.__cs1__}{self.__cs2__}ETX".format(
            self=self
        )
...
class PacketFactory:
    ...
    @classmethod
    def create_query(
        self, setting: str = None, unique_id: str = None, checksum: str = None
    ) -> QueryPacket:
        ...
        """
        if setting is None and unique_id is not None:
            logger.debug(
                "Searching for setting with unique_id {unique_id}...",
                unique_id=unique_id,
            )
            for setting, values in settings_dict.items():
                if values["unique_id"] == unique_id:
                    logger.debug("Found setting {setting}!", setting=setting)
                    return self.create_query(setting=setting, checksum=checksum)
            else:
                raise ValueError("Cannot find setting with unique_id:", unique_id)

        logger.debug(
            "Creating query packet for setting {setting} with checksum {checksum}",
            setting=setting,
            checksum=checksum,
        )
            ...
            for setting, values in settings_dict.items():
                if values["unique_id"] == unique_id:
                    ...
                    return self.create_command(
                        setting=setting, value=value, checksum=checksum
                    )
            else:
                raise ValueError("Cannot find setting with unique_id:", unique_id)

        logger.debug(
            "Creating command packet for setting {setting} with value {value} and checksum {checksum}",
            setting=setting,
            value=value,
            checksum=checksum,
        )
    ...
    def create_from_response(self, raw: bytes) -> Packet:
        ...
        """
        logger.debug("Creating packet from bytearray {raw}...", raw=raw)
        if raw[0:1] != STX and raw[len(raw) - 1 : len(raw)] != ETX:
            raise ValueError(
                "invalid bytearray, it must start with STX (\x02) and end with ETX (\x03):",
                raw,
            )
        ...
        if _type == TYPE_REPLY_PACKET:
            ...
            logger.debug("Packet seems to be response packet, creating instance.")
