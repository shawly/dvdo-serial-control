...
language = config['global']['lang']

# Load dictionaries
settings_dict = yaml.safe_load(
    open('data/settings_dict_{lang}.yaml'.format(lang=language)))
errors_dict = yaml.safe_load(
    open('data/errors_dict_{lang}.yaml'.format(lang=language)))
...
error_matcher = re.compile(
    b'\\x02(?P<cmd>(\d)(\d))(?P<cnt>(\d)(\d))(?P<err>([0-9])([0-9]))\\x00(?P<chk>([A-F0-9])([A-F0-9]))\\x03')
